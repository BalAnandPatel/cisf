
<?php
include "include/header.php";
?>
<!-- breadcrumb Wrapper Start -->
<div class="breadcrumb-wrapper">
  <div class="container">
    <div class="banner-title">
      <h2>Contact Us</h2>
      <div class="line"> <span></span></div>
    </div>
    <ul class="inner-breadcrumb">
      <li><a href="index.php">Home</a></li>
      <li>Contact Us</li>
    </ul>
  </div>
</div>
<!-- breadcrumb Wrapper End --> 
<!-- Inner page Wrapper Start -->
<div class="inner-page-wrapper contacts-wrapper">
  <div class="container">
    <div class="row">
      <div class="col-md-6 col-sm-12">
        <div class="contactus-txt">
          <h2>Official Details</h2>
        </div>
        <div class="contact-address">
          <div class="contact-left"> <i class="fa"><img src="images/" alt=""></i>
            <div class="address">
              <h3>Office Address</h3>
              1/4798 Galii No.12 Balbir Nagar <span>Extension Delhi 110032</span>
               <span>Email: info@voiceofcisfretiree.in</span> </div>
          </div>
          <div class="contact-left"> <i class="fa"><img src="images/" alt=""></i>
            <div class="address">
              <h3>Contact Detail</h3>
              Shri JB Kaushik (President):-<br>9900807556</span>
              <span>Kadam Raj Singh (General Secretary):-<br>7234038830</span> </div>
          </div>
          <!-- <div class="contact-right"> <i class="fa"><img src="images/contact-icon2.jpg" alt=""></i>
            <div class="address">
              <h3>New York</h3>
              Porto Blvd, Suite 100 <span>Los Angeles/CA</span> <span>Phone: 123-456-7890</span> </div>
          </div> -->
        </div>
      </div>
      <div class="col-sm-6 hidden-sm"> <img src="images/logo.png" alt=""> </div>
    </div>
  </div>
</div>
<!-- Inner page Wrapper End --> 
<!-- Contact Form Wrapper Start -->
<!-- <div class="contact-form-wrapper">
  <div class="google-map">
    <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d198710.35112897935!2d-98.51489117772236!3d38.904562823631146!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x54eab584e432360b%3A0x1c3bb99243deb742!2sUnited+States!5e0!3m2!1sen!2sin!4v1471865832140" allowfullscreen=""></iframe>
  </div>
  <div class="contact-info">
    <div class="container">
      <div class="form-bg">
        <div class="row">
          <div class="col-sm-12">
            <div class="contact-form">
              <h2>Send Us a Message</h2>
              <div class="row">
                <form>
                  <div class="col-md-4 col-sm-12">
                    <div class="form-group">
                      <input class="form-control" id="name" placeholder="Your Name" required type="text">
                    </div>
                  </div>
                  <div class="col-md-4 col-sm-12">
                    <div class="form-group">
                      <input class="form-control" id="phone" placeholder="Your Phone number" required type="text">
                    </div>
                  </div>
                  <div class="col-md-4 col-sm-12">
                    <div class="form-group">
                      <input class="form-control" id="email" placeholder="Enter Your E-Mail" required type="email">
                    </div>
                  </div>
                  <div class="col-sm-12">
                    <div class="form-group">
                      <textarea class="form-control" id="comment" placeholder="Comment here..." required></textarea>
                    </div>
                  </div>
                  <div class="col-sm-12 text-right">
                    <button type="submit" class="btn-one">Submit</button>
                  </div>
                </form>
              </div>
            </div>
          </div> -->
          <!-- <div class="col-sm-4">
            <div class="why-us">
              <h2>Why Us?</h2>
              <ul>
                <li>Accounting</li>
                <li>Finance</li>
                <li>Staffing</li>
                <li>Finance</li>
                <li>Staffing</li>
              </ul>
              <ul>
                <li>Accounting</li>
                <li>Finance</li>
                <li>Staffing</li>
                <li>Finance</li>
                <li>Staffing</li>
              </ul>
            </div>
          </div> -->
        </div>
      </div>
    </div>
  </div>
</div>
<!-- Contact Form Wrapper End --> 

<?php
include "include/footer.php";
?>